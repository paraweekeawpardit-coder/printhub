...
export default function OrderReviewPage() {
  ...
  // รองรับทั้ง [order_id] และ [orderId] ป้องกันปัญหาการตั้งชื่อโฟลเดอร์ไม่ตรงกัน
  const rawOrderId = params?.order_id || params?.orderId;
  const activeOrderId = Array.isArray(rawOrderId) ? rawOrderId[0] : (rawOrderId as string);
  ...
  // ดึงข้อมูลคำสั่งซื้อจริงจาก Backend API
  ...
  const handleSubmitReport = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && result.success) {
        ...
        alert("เกิดข้อผิดพลาด: " + (result.message || "ไม่สามารถส่งรายงานได้"));
      }
    } catch (err) {
      ...
      <header className="bg-white border-b border-slate-200/80 sticky top-0 z-30">
        <div className="max-w-4xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          {activeOrderId && activeOrderId !== "undefined" && (
            <span className="text-xs text-slate-400 font-mono">Order #{activeOrderId.slice(0, 8)}</span>
          )}
        </div>
      </header>
